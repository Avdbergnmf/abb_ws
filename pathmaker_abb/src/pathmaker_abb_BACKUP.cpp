...
void commClass::init()
{
    ...
    // init publishers
    joint_traj_act_goal_pub = nh.advertise<control_msgs::FollowJointTrajectoryActionGoal>("/joint_trajectory_action/goal", 10);
...
void commClass::publish_goal()
{
    ...
    if (link_arm) {
        ...
        // send to Rviz
        brtest.sendTransform( TF_geo_pose );
    }
    else {
        // Geo Stuff
    ...
    double timeout = 0.02; // timeout per attempt
